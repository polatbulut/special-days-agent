...
- **Fail-soft scoring:** `LLMScorer.score` never aborts the batch. If the model call raises (network/HTTP error, throttling/429) or `_parse` raises on an empty/unparseable reply, that **one record** is scored by the heuristic instead (and a warning is logged). So a single bad reply in a large run degrades gracefully rather than discarding every other scored row — matching the fail-soft collection stage. The scoring step still runs concurrently (`ThreadPoolExecutor`, default 8 for LLM); to actually *get* LLM scores rather than fallbacks, keep `--concurrency` under the deployment's rate limit and give reasoning models a large enough `AZURE_OPENAI_MAX_COMPLETION_TOKENS` (an exhausted budget returns empty content).
